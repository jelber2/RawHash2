...
{       
        *n_ = 0;
        ...
        while (pl->fp) {
                ...
                /* no_sig_filter=1 only for ext_peaks: peak indices reference raw sample
                 * positions in the unfiltered signal.  For ext_events the signal is only
                 * used to compute z-score statistics, so apply the same 30-200 pA quality
                 * filter used during index building. */
                ri_read_sig(pl->fp, s, 1, pl->opt->ext_peaks ? 1 : 0);
                size += s->l_sig;
